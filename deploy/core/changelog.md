...
* CHANGED: **The bespoke nREPL middleware is gone.** 1,192 lines of Clojure that reimplemented completion, documentation lookup, stacktrace formatting and a ClojureScript compiler driver are replaced by `cider-nrepl` and `orchard` — what CIDER, Calva, Conjure and vim-iced all sit on. The result is better, not merely cheaper to own: the stacktrace is orchard's structured frame list and names the file and line *in the editor*, because `:file`, `:line` and `:column` go out with every form; completion is compliment, which understands locals
* ADDED: **ClojureScript evaluation**, through the same path as Clojure — forms from the parse tree, a result beside each one, watches. A `.cljs` file evaluates in a second nREPL session on the same connection, so a project's Clojure and ClojureScript files both keep working. Which environment is not a question: a `shadow-cljs.edn` beside the code means shadow's own, anything else means piggieback over node, and Light Table injects piggieback and the compiler the way it injects cider-nrepl
* CHANGED: Evaluating a `.cljs` file no longer goes looking for a browser to attach to first — it threw before ever reaching the REPL. Connecting a browser is a thing you choose, in the Connect bar
* ADDED: **The editor changes itself while running again.** Put the cursor in a form in a Light Table plugin or in Light Table's own source, evaluate it, and the editor you are typing in changes — a new command exists, a behavior does something else, the window restyles. This is the feature Light Table is named for and it had not worked in this fork: the receiving half was intact, but the ClojureScript compiler that fed it was deleted with the old middleware and nothing replaced it. Verified in a packaged release, not just from the tree — `.js`, `.css` and `.cljs` buffers, four forms and four inline results, a command defined at runtime and then run
* ADDED: A ClojureScript compiler *in the window*, so that needs no project, no JVM and no jack-in. shadow-cljs's `:target :bootstrap` emits the per-namespace analysis a compiler needs to resolve `lt.objs.editor/->val` to the function the window is already holding; `shadow.cljs.bootstrap.browser` loads that analysis and deliberately skips the code, which is what stops it re-running every top-level `def` and swapping out the atoms the running editor holds. It works in a release because the bundle is built `:optimizations :simple` with `:output-wrapper false`, so the whole `lt.*` API is a live global — the build carries the feature
* CHANGED: A `.cljs` file picks its client from the file rather than from a setting: a `plugin.edn` above it, a `project.clj` naming `lighttable`, or a namespace starting with `lt.` evaluates into the window; anything else is someone's project and goes to that project's REPL. `doc/workflow.md` has told people to choose `Light Table UI` from a menu since 2014, and there is no longer a menu to choose it from
* ADDED: Four smoke checks over that path — the compiler starts, evaluating a buffer defines a command that did not exist, through the client that is this window, with a result beside each top-level form. 80 checks, up from 76
* CHANGED: ClojureScript results had a different shape from Clojure ones — a single `:result` rather than one per top-level form — because the only thing that used to answer them was a browser evaluating one expression. One shape now, with the old one still understood, so the browser route keeps working
* ADDED: **Rename a symbol across the project**, as one action, with one undo. `lt.objs.workspace-edit` is the primitive underneath and is now the only way Light Table changes text it is not showing — a file that is open is edited through its editor so the buffer and the disk agree, a file that is not is read and written, and both are recorded so `Editor: Undo workspace edit` takes back all of it. It refuses to start if any file it would touch has unsaved changes, because what it took away is what it has to be able to put back
* FIXED: The LSP framing layer keywordised every JSON key, and `WorkspaceEdit.changes` is the one place the protocol keys a map by arbitrary text — a `file://` URI became a keyword whose namespace was `file:`, and that is what came back out. Keys that are identifiers become keywords and everything else stays a string, which is a rule about the key rather than a special case in whichever surface trips over it next
* ADDED: **Find references** and **list this file's symbols**, from any language server, into the search sidebar — both are lists of places in the project and Light Table already had one. clojure-lsp finds uses in files that are not open, because it has indexed the project
* ADDED: **Completion, documentation and jump-to-definition from any language server**, not just diagnostics. Generic, so every language with a server gets them at once — TypeScript completion is 1002 entries deep where it used to be the words already in the buffer. Which surfaces an editor gets comes from the server's own `initialize` result: `hoverProvider` grants the `:docable` tag, `documentSymbolProvider` grants `:navigable`, so a language earns a surface by saying it can answer
* ADDED: Where a language has both a server and a REPL, Light Table's own design settles it. Completion **merges** — `:hints+` is a reduction, so the server's suggestions and the REPL's appear together. Documentation and jump-to-definition **defer to a connected REPL**, because there is one doc bar and one cursor and the REPL knows what is actually loaded. Diagnostics are the server's unconditionally: no REPL has any
* ADDED: A result appears beside **each top-level form**, and that no longer depends on the REPL. `lt.objs.editor.treesitter/top-level-forms` splits the buffer from the parse tree that is already current on every keystroke — so Light Table knows where the forms are whether or not anything is connected. It used to be the middleware's job, which meant inline results needed a bespoke server per language
* FIXED: `lt.objs.editor/->token` returned a map whose every field was nil. CodeMirror builds a token with `new Token(…)` and `js->clj` converts *plain* objects only, so a class instance came back unchanged — which took out `find-symbol-at-cursor`, and with it documentation and jump-to-definition, for every language that asked
* FIXED: A file evaluated form by form defined all of its vars in `user`. nREPL resolves a request's `:ns` when the request arrives rather than when it runs, so sending every form at once had each one answered `namespace-not-found` before the `(ns …)` that creates the namespace had been dequeued. Forms are evaluated one at a time now, in order, each result appearing as its form finishes — which is what watching a file evaluate should look like anyway
...
* ADDED: A Language Server Protocol client. Diagnostics appear inline, under the line they are about, using the same mechanism as inline eval results. `typescript-language-server` and `clojure-lsp` are configured out of the box, found in the project's `node_modules/.bin` or on `PATH`; a project that has not installed one is unaffected. **Language server: Status for this editor** says why, when nothing appears. See [doc/lsp-architecture.md](https://github.com/TrevorS/LightTable/blob/develop/doc/lsp-architecture.md)
...
* FIXED: Saving did nothing. A stat crossing the preload boundary is plain data, so `check-mtime` read an undefined `mtime`, threw inside the behavior that raised it, and `save` aborted before writing — leaving the tab dirty and the reason in the console. Present since 0.9.0. Detecting a file changed outside the editor used the same function
