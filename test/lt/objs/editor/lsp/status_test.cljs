(ns lt.objs.editor.lsp.status-test
...
(deftest the-six-phases-are-what-they-say
  (is (= :no-file (status/phase (assoc answering :path nil))))
  (is (= :unconfigured (status/phase (assoc answering :language-id nil))))
  (is (= :ready (status/phase answering)))
  (is (= :starting (status/phase (assoc answering :ready? false))))
  (is (= :missing (status/phase (assoc answering :connected? false :ready? false
                                       :found nil))))
  (is (= :idle (status/phase (assoc answering :connected? false :ready? false)))))
...
(deftest a-file-with-no-server-declared-is-not-a-problem
  ...
  (let [s (assoc answering :language-id nil)]
    (is (= "No language server is configured for this file type." (status/line s)))
    (is (nil? (status/indicator s)) "nothing to draw, so nothing is drawn"))
  (is (nil? (status/indicator (assoc answering :path nil)))))
...
(deftest a-second-declared-server-cannot-describe-a-live-connection
  ...
  (let [two (merge answering
    ...
    (is (= :finished (:status (status/indicator two))))
...
(deftest starting-is-distinguishable-from-absent
  ...
  (let [s (assoc answering :ready? false)]
    (is (= "Starting /p/node_modules/.bin/vtsls …" (status/line s)))
    (is (= :connecting (:status (status/indicator s)))))
  (testing "and it says the command when the binary was never located"
    (is (= "Starting vtsls …"
           (status/line (assoc answering :ready? false :found nil))))))
...
(deftest missing-splits-into-the-two-things-to-go-and-do
  (let [absent (assoc answering :connected? false :ready? false :found nil)]
    (is (= "No project root above /p/a.ts — looked for tsconfig.json"
           (status/line (assoc absent :root nil)))
  ...
  (is (= :lost (:status (status/indicator (assoc answering :connected? false
...
(deftest installed-and-unconnected-is-its-own-answer
  (let [s (assoc answering :connected? false :ready? false)]
    (is (= "Found /p/node_modules/.bin/vtsls, but this editor is not connected to it."
           (status/line s)))
    (is (= :queued (:status (status/indicator s))))))

(deftest a-diagnostic-count-is-pluralised-and-carried
  (is (= "Connected to /p/node_modules/.bin/vtsls — 1 diagnostic on screen"
         (status/line (assoc answering :diagnostics 1))))
  (is (= "Connected to /p/node_modules/.bin/vtsls — 3 diagnostics on screen"
         (status/line (assoc answering :diagnostics 3))))
  (is (= 3 (:diagnostics (status/indicator (assoc answering :diagnostics 3))))
