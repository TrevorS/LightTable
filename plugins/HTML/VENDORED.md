...
| `codemirror/htmlmixed.js and codemirror/htmlembedded.js` | core bundles current `htmlmixed` and `htmlembedded` modes now — see `script/gen-codemirror-requires.js` — so the copy was an old mode overwriting a new one at load |
