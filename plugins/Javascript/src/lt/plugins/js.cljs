...
;; plugins/Javascript/VENDORED.md.
(declare js-lang)
...
(defn ->body [^js tree]
  (if (> 1 (count (.-body tree)))
    (.-body tree)
    (let [^js node (aget (.-body tree) 0)]
      (if (and (= (.-type node) "ExpressionStatement")
               (.. node -expression -callee)
               (.. node -expression -callee -body))
        ;;we have a wrapping function
        (.. node -expression -callee -body -body)
        (.-body tree)
      ))))
...
(defn by-pos [locs pos]
  (let [line (:line pos)
        covers? (fn [form]
                  (let [^js loc (:loc form)]
                    (and (<= (.. loc -start -line) line)
                         (>= (.. loc -end -line) line))))]
    (first (filter covers? locs))))
...
(browser/add-util :watch (fn [exp ^js meta]
